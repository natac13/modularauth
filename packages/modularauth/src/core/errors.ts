/**
 * A list of errors that can be thrown by ModularAuth.
 *
 * You can use these errors to check the type of error and handle it. For example.
 *
 * ```ts
 * import { InvalidAuthorizationCodeError } from "@modularauth/core/errors"
 *
...
/**
 * Invalid grant error for authorization code issues.
 */
export class InvalidGrantError extends OauthError {
  constructor(description = "Invalid authorization code") {
    super("invalid_grant", description)
  }
}

/**
 * Convert an OauthError to an HTTP Response
 */
export function oauthErrorResponse(error: OauthError, status = 400): Response {
  return new Response(
    JSON.stringify({
      error: error.error,
