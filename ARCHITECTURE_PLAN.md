...
- **Lets you manage your own database** with user management via onSuccess callback
...
const auth = createAuth<
  ...
  User
>({
  // Storage adapter - just like OpenAuth
  ...
  // Handle successful authentication - YOU manage users
  onSuccess: async (ctx, provider, providerData) => {
    // providerData is typed based on the provider
    let user = await yourDB.findUser({
      provider: provider,
      providerId: providerData.id
    })
    
    if (!user) {
      // New user signup flow - YOUR database, YOUR logic
      user = await yourDB.createUser({
  ...
  onRefresh: async (subject) => {
    // Get current state from YOUR database
  ...
  contextSwitch: {
    enabled: true,
    async handler(currentSubject, newContext) {
      // Validate and switch workspace atomically - YOUR logic
      const user = await yourDB.switchWorkspace(
...
## Storage Interface - OpenAuth's Proven Pattern

```typescript
// Use OpenAuth's simple, proven storage interface
...
// Configuration uses storage + onSuccess for user management
interface ModularAuthConfig {
  // Just storage - no complex adapters
  ...
  // YOU handle user management - not the library
  onSuccess(
    ctx: SuccessContext,
    provider: string,
  ...
  // Optional: refresh logic for dynamic data
  onRefresh?(subject: Subject): Promise<Subject>
  
  // Optional: OIDC userInfo endpoint
  userInfo?(subject: Subject): Promise<OIDCUserInfo>
  
  // Optional: context switching
  contextSwitch?: {
    enabled: boolean
    handler(subject: Subject, newContext: any): Promise<Subject>
  }
}

// Storage handles auth data with simple keys:
// ['oauth:refresh', token] - refresh tokens
// ['oauth:code', code] - authorization codes  
// ['otp:code', email, code] - OTP codes
// ['passkey:credential', credentialId] - passkey credentials
// ['jwt:keys', keyId] - signing keys

// YOU handle user data in your own database however you want!
```
...
class TokenHandler {
  async handle(request: Request): Promise<Response> {
    ...
    if (grantType === 'refresh_token') {
      ...
      const updatedSubject = await this.config.onRefresh(subject)
...
3. Use OpenAuth's proven storage interface
...
### Phase 4: Storage Implementations (Week 4)
1. Create DynamoDB storage implementation
2. Use OpenAuth's MemoryStorage for testing
3. Document storage interface for community contributions
4. Add storage testing suite
